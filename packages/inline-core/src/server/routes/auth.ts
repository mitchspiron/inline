 ...
 */
import { clearedCookies, createSession, sessionCookies } from '../auth';
import { LIMITS, declaredBodyTooLarge, guardRate, json, type FunctionContext } from '../guard';
...
export function createAuthRoute() {
  return {
    ...
    async onRequestPost({ request, env }: FunctionContext): Promise<Response> {
    ...
    async onRequestDelete(): Promise<Response> {
      const response = json({ ok: true });
